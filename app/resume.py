...
def generate_resume(name, addr, em, mob, obj, ad, xp, line, all,title,text):
    try:
    ...
    # function to insert horizontal line
    ...
    titlesection(doc, name, addr, em, mob)
    careerobj(doc,obj)
    education(doc,ad)
    skills(doc,line)
    workxp(doc,xp)
    projects(doc,all)
    section(doc,title,text)
    save(doc)
